...
# ---------- IMPORTANT NOTICE about the limitation ----------
st.info(
    "📌 **Important:** This model was trained on only two categories — **cat** and **dog**. "
    "If you upload anything else (a person, an object, a screenshot), the model will still "
    "predict Cat or Dog because those are the only two options it knows. "
    "For accurate results, please upload a clear photo of a cat or a dog."
)
...
# ---------- File uploader ----------
uploaded = st.file_uploader("Choose a photo of a cat or a dog", type=["jpg", "jpeg", "png"])
...
if uploaded is not None and model_ok:
    ...
    if img_color is None or img_gray is None:
        ...
        # Reminder under the result
        st.caption(
            "💡 Remember: the model only knows cat and dog. High confidence does not "
            "always mean correct — for non-animal photos, it will still pick one."
        )
